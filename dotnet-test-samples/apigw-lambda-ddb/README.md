...
This pattern creates an Amazon API Gateway HTTP API, a AWS Lambda function and a DynamoDB Table using SAM and .NET 6.
...
## Description
The SAM template contains all the information to deploy AWS resources(an API Gateway, 4 Lambda functions and a DynamoDB table) and also the permission required by these service to communicate.
...
The application follows Clean/Hexagonal/Ports & Adapters Architecture principles. Meaning core business logic has no external dependencies and all integrations are pushed to the outside. The solution is split down into 3 projects:
...
The goal of these tests is to run a unit test on the handler method of the Lambda functions. It uses [Moq](https://github.com/moq/moq4) as a mocking framework. The `ProductsDAO` interface is mocked.
...
It uses the [IClassFixture](https://xunit.net/docs/shared-context) feature of [xUnit](https://xunit.net/) to perform [setup](./tests/ApiTests.IntegrationTest/Setup.cs) and cleardown logic. The [setup](./tests/ApiTests.IntegrationTest/Setup.cs) code retrieves the API URL and DynamoDB table name from the deployed CloudFormation stack. Cleardown code ensures all created Product Id's are deleted from the table.

The tests themselves make API calls directly to the deployed resource. All API calls are held in the [ApiDriver](./tests/ApiTests.IntegrationTest/Drivers/ApiDriver.cs) class to ensure any changes to API schemas/routes can be centrally managed.

...
class TestFileProcessingWorkflow(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.assertTrue(dynamodb_outputs, f"Cannot find DynamoDBTableName in {stack_name}")
        self.dynamodb_table_name = dynamodb_outputs[0]["OutputValue"]

        # UnicornInventoryBucket
        inv_bucket_outputs = [output for output in stack_outputs
                              if output["OutputKey"] == "UnicornInventoryBucket"]
        self.assertTrue(inv_bucket_outputs, f"Cannot find UnicornInventoryBucket in {stack_name}")
    ...
    def tearDown(self) -> None:
        ...
        # And remove the test data from the locations
    ...
    def test_file_processor_happy_path(self):
        ...
        self.assertEqual(item_should_not_be_found["Count"], 0,
                         "Data exists before test, invalid starting state")
        ...
        test_data_key = f"INTEGRATION_TEST/TEST{self.id_postfix}.test_file_processor_happy_path.csv"
        ...
        self.assertEqual(item_should_be_found["Count"], 1,
                         "More than one item found after insert of data")

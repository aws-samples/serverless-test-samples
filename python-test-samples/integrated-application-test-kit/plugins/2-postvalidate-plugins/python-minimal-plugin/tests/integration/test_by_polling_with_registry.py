...
class TestMinimalPluginByPollingWithRegistry(TestCase):
    """
    ...
    event_registry_name: str = None
    plugin_tester_trigger_event_schema_name: str = None
    ...
    def setUp(self):
        ...
        self.event_registry_name = stack_outputs.outputs["PluginEventRegistryName"]
        self.plugin_tester_trigger_event_schema_name = stack_outputs.outputs["PluginTesterWorkflowTriggerEventSchemaName"]
    ...
    def test_minimal_plugin_event_published_polling_registry(self):
        """
        Test case to validate that our plugin (which runs in response to the 
        video system's "postvalidate" event) emits the correct event to the
        event bus after processing.
        ...
        ## Prepare our trigger event. Pull the schema from EventBridge Schema
        ## Registry, then set test-specific properties.
        def set_trigger_event_properties(event):
            event["eventHook"] = "postValidate"
            event["pluginTitle"] = "PythonMinimalPlugin"
            return event
        trigger_event = self.iatk_client.generate_mock_event(
            registry_name=self.event_registry_name,
            schema_name=self.plugin_tester_trigger_event_schema_name,
            contexts=[set_trigger_event_properties],
        ).event
